#include "Scene/GameScene/Room_1.h"
USING_NS_CC;
...
bool Room_1::init()
{
	...
	//使用Tiled生成的.tmx文件去创建一个TMXTiledMap类的对象，再将这个tileMap添加到场景
	auto tileMap = TMXTiledMap::create("GameScene/Room_1/Room_1.tmx");
	...
	auto objectWall_20 = objectsLayer->getObject("wall_20");
	auto objectWall_21 = objectsLayer->getObject("wall_21");
	auto objectWall_22 = objectsLayer->getObject("wall_22");
	auto objectWall_23 = objectsLayer->getObject("wall_23");
	auto objectWall_24 = objectsLayer->getObject("wall_24");
	...
	//Tiled支持自行设置对象的属性，其中"x","y"是自带的属性，"Length"和"Width"是自行添加的属性
	...
	this->vecObstacle.push_back(Rect::Rect(objectWall_20["x"].asFloat(), objectWall_20["y"].asFloat(), objectWall_20["Length"].asFloat(), objectWall_20["Width"].asFloat()));
	this->vecObstacle.push_back(Rect::Rect(objectWall_21["x"].asFloat(), objectWall_21["y"].asFloat(), objectWall_21["Length"].asFloat(), objectWall_21["Width"].asFloat()));
	this->vecObstacle.push_back(Rect::Rect(objectWall_22["x"].asFloat(), objectWall_22["y"].asFloat(), objectWall_22["Length"].asFloat(), objectWall_22["Width"].asFloat()));
	this->vecObstacle.push_back(Rect::Rect(objectWall_23["x"].asFloat(), objectWall_23["y"].asFloat(), objectWall_23["Length"].asFloat(), objectWall_23["Width"].asFloat()));
	this->vecObstacle.push_back(Rect::Rect(objectWall_24["x"].asFloat(), objectWall_24["y"].asFloat(), objectWall_24["Length"].asFloat(), objectWall_24["Width"].asFloat()));
	...
	this->Vec2LeftPosition = Vec2(objectLeftPosition["x"].asFloat() + 2.0f, objectLeftPosition["y"].asFloat());
	this->Vec2RightPosition = Vec2(objectRightPosition["x"].asFloat() - 2.0f, objectRightPosition["y"].asFloat());

	return true;
}

void Room_1::update(float delta)
{
	//调用父类的update函数
	BasicScene::update();
	if (!this->_ifGameEnd)
	...
	ifRoom1Ended = 1;
	//判断是否进入门
	//用来判断是否进门的新Rect，即偏左或者偏右时是否进入
	//Rect(_role->getRect().origin + Vec2(-3, 0), _role->getRect().size);
	
	//if (_role->getRect().intersectsRect(rectDoor_left))
	if(Rect(_role->getRect().origin + Vec2(-8, 0), _role->getRect().size).intersectsRect(rectDoor_left))
	{
			...
			nextScene->setInitialLocation("right");
			Director::getInstance()->replaceScene(nextScene);
	}
	if (Rect(_role->getRect().origin + Vec2(8, 0), _role->getRect().size).intersectsRect(rectDoor_right))
	{
			_role->stopActionByTag(104);
			_role->setPosition(Vec2(_role->getPosition().x - 6.67f, _role->getPosition().y));
			updateRoleAttribution(this->_role->getBlood(), this->_role->getShield(), this->_role->getBlue());
			auto nextScene = Room_0::create();
